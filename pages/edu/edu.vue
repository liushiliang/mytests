<template>
	<view>
		<return-btn ttl="教育" :radio="bgRadio" :isCenter="true" @returnBtnInitial="returnBtnInitial" v-if="is_outer" :usingRedirect="true" url="/pages/index/index"></return-btn>
		<return-btn ttl="教育" :radio="bgRadio" :isCenter="true" @returnBtnInitial="returnBtnInitial" v-else></return-btn>
		...
		<!-- 实际内容 start -->
		<div class="container fr-space-between" v-if="curIndex==id" v-for="it,id in real_data" :style="{'padding-top':listsPT+'px'}">
			<div class="edu-item" v-for="item,index in it.data" :data-id="item.id"  data-ttl="教育"  @click="toDetail">
				<image class="item-img" mode="aspectFill" :src="item.thumb"></image>
				<div class="item-h "><div class="show_two" style="width:343rpx;">{{item.title}}</div></div>
		...
		<div class="black-wrapper" @touchmove.stop.prevent="touchMoveHandler" @click="hideSNB" v-if="isShowWrapper" :class="{'black-wrapper-show':isShowSNB}">asdsadsa</div>
		 <div class="super-nav-block"  :style="{top:snTop+'px'}">
			 ...
			 <div class="sn-btns fr-start">
				 <div class="sn-item" :class="{'sn-item-selected':curIndex==0?true:false}" @click="setBtnAndGo(0)">热点动态</div>
				 <div class="sn-item" :class="{'sn-item-selected':curIndex==1?true:false}" @click="setBtnAndGo(1)">学校快讯</div>
				 <div class="sn-item" :class="{'sn-item-selected':curIndex==2?true:false}" @click="setBtnAndGo(2)">教育培训</div>
				 <div class="sn-item" :class="{'sn-item-selected':curIndex==3?true:false}" @click="setBtnAndGo(3)">早教学堂</div>
				 <div class="sn-item" :class="{'sn-item-selected':curIndex==4?true:false}" @click="setBtnAndGo(4)">家庭教育</div>
				 <div class="sn-item" :class="{'sn-item-selected':curIndex==5?true:false}" @click="setBtnAndGo(5)">高考</div>
				 <div class="sn-item" :class="{'sn-item-selected':curIndex==6?true:false}" @click="setBtnAndGo(6)">中小学</div>
				 <div class="sn-item" :class="{'sn-item-selected':curIndex==7?true:false}" @click="setBtnAndGo(7)">考试</div>
				 <div class="sn-item" :class="{'sn-item-selected':curIndex==8?true:false}" @click="setBtnAndGo(8)">开讲啦</div>
				 <div class="sn-item" :class="{'sn-item-selected':curIndex==9?true:false}" @click="setBtnAndGo(9)">名校来了</div>
			 </div>
...
.edu-item{
	...
	.item-h{
		padding-top: 29-8/2-3/2rpx;
		font-size: 30rpx;
		color: #474747;
		padding-bottom: 20rpx;
		padding-left: 8rpx;
		max-width:343rpx;
		line-height: 28+8+4rpx;
		height: (28+8)*2rpx;
		font-size: 32rpx;
	}
	.item-h2{
